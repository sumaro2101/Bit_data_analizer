...
class LogAnalyzer:
    ...
    #         leng_step = len(expected_step)
    #         if leng_step == 1 and expected_step[0].name in ['-',
    #                                                         'nan',
    #                                                         '']:
    #             continue
    #         end += leng_step
    #         if new_interval_step:
    #             end += len(new_interval_step) - len(interval_actual_event)
    #             new_interval_step = []
    #         try:
    #             next_step_first = self._get_next_step(
    #                 expected_steps=expected_steps,
    #                 step_index=step_index,
    #             )
    #             next_name_event = next_step_first[0].name
    #         except IndexError:
    #             interval_actual_event = actual_events[start:]
    #             actual_step = Step(
    #                 step_number=expected_step.step_number,
    #                 action=expected_step.action,
    #                 events=interval_actual_event,
    #             )
    #             list_of_steps.append(actual_step)
    #             start = end
    #             continue
    #         interval_expected_names = [event.name
    #                                    for event
    #                                    in expected_step]
    #         interval_next_names = [event.name
    #                                for event
    #                                in actual_events[end - 1:]]
    #         start_expected_event = expected_step[0]
    #         end_expected_event = expected_step[-1]
    #         interval_actual_event = actual_events[start:end]
    #         try:
    #             closest_next_first_event = self._get_next_step_name(
    #                 expected_steps=expected_steps,
    #                 step_index=step_index,
    #                 next_names=interval_next_names,
    #             )
    #             closest_last_event = interval_next_names.index(end_expected_event.name)
    #         except ValueError:
    #             if (leng_step == 1
    #                and interval_actual_event
    #                and start_expected_event.name == interval_actual_event[0].name):
    #                 actual_step = Step(
    #                     step_number=expected_step.step_number,
    #                     action=expected_step.action,
    #                     events=interval_actual_event,
    ...
    #                 start = end
    #                 continue
    #             prev_events = [event.name for event in interval_actual_event]
    #             if next_name_event not in prev_events:
    #                 actual_step = Step(
    #                     step_number=expected_step.step_number,
    #                     action=expected_step.action,
    #                     events=[],
    ...
    #                 continue
    #             try:
    #                 last_index = prev_events[::-1].index(next_name_event) + 1
    #             except ValueError:
    #                 actual_step = Step(
    #                     step_number=expected_step.step_number,
    #                     action=expected_step.action,
    ...
    #                 end -= leng_step
    #                 continue
    #             new_interval = interval_actual_event[:len(interval_actual_event) - last_index]
    #             actual_step = Step(
    #                 step_number=expected_step.step_number,
    #                 action=expected_step.action,
    #                 events=new_interval,
    ...
    #             start = end
    #             continue
    #         equal_start = interval_actual_event[0].name == start_expected_event.name
    #         equal_end = interval_actual_event[-1].name == end_expected_event.name
    #         match equal_start, equal_end:
    #             case True, True:
    #                 if (closest_next_first_event - closest_last_event) == 1:
    #                     actual_step = Step(
    #                         step_number=expected_step.step_number,
    #                         action=expected_step.action,
    #                         events=interval_actual_event,
    ...
    #                     start = end
    #                 else:
    #                     if closest_last_event > closest_next_first_event:
    #                         interval_actual_event = actual_events[start:(end - 1) + closest_last_event]
    #                         actual_step = Step(
    #                             step_number=expected_step.step_number,
    #                             action=expected_step.action,
    #                             events=interval_actual_event,
    #                         )
    #                         list_of_steps.append(actual_step)
    #                         start += len(interval_actual_event)
    #                         end = start
    #                     elif closest_next_first_event > closest_last_event:
    #                         interval_names = [event.name
    #                                           for event
    #                                           in interval_actual_event]
    #                         if not interval_expected_names == interval_names:
    #                             interval_actual_event = actual_events[start:(end - 1) + closest_next_first_event]
    #                         actual_step = Step(
    #                             step_number=expected_step.step_number,
    #                             action=expected_step.action,
    #                             events=interval_actual_event,
    ...
    #                         end = start
    #                     else:
    #                         interval_actual_event = actual_events[start:end + closest_last_event]
    #                         actual_step = Step(
    #                             step_number=expected_step.step_number,
    #                             action=expected_step.action,
    #                             events=interval_actual_event,
    ...
    #                         end = start
    #             case _:
    #                 if (leng_step == 1 and (next_name_event != interval_actual_event[0].name or
    #                                         next_name_event != start_expected_event.name)):
    #                     actual_step = Step(
    #                         step_number=expected_step.step_number,
    #                         action=expected_step.action,
    #                         events=[],
    ...
    #                     end = start
    #                     continue
    #                 elif closest_next_first_event == 0 and (next_name_event == start_expected_event.name or
    #                                                       next_name_event == interval_actual_event[0].name):
    #                     new_interval_step = actual_events[start:end + 1]
    #                     closest_next_first_event = len(interval_actual_event) + 1
    #                 elif closest_next_first_event == leng_step and end_expected_event.name == next_name_event:
    #                     closest_next_first_event = interval_next_names[leng_step:].index(end_expected_event.name) + leng_step + 1
    #                     new_interval_step = actual_events[start:end + closest_next_first_event]
    #                 else:
    #                     new_interval_step = actual_events[start:end + closest_next_first_event]
    #                 actual_step = Step(
    #                     step_number=expected_step.step_number,
    ...
    #                 )
    #                 list_of_steps.append(actual_step)
    #                 start = start + len(new_interval_step)
    #                 if not new_interval_step:
    #                     end -= leng_step
    #     return list_of_steps
    ...
    #     step_index += 1
    #     try:
    #         next_step = expected_steps[step_index]
    #     except IndexError:
    #         return None
    #     if next_step[0].name in ['',
    #                              'nan',
    #                              '-']:
    ...
    #             step_index=step_index,
    #         )
    #     if next_step[0].name not in ['',
    #                                  'nan',
    #                                  '-']:
    #         return next_step

    # def _get_next_step_name(self,
    #                         expected_steps: list[Step],
    #                         step_index: int,
    #                         next_names: list[str],
    #                         ) -> str:
    ...
    #         return 1
    #     try:
    #         index = next_names.index(next_step[0].name)
    #     except ValueError:
    #         index = self._get_next_step_name(
    #             expected_steps=expected_steps,
    #             step_index=step_index,
